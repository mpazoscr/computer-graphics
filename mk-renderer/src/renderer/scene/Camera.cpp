  ...
  namespace renderer
  {
    namespace scene
    {
      ...
      void FPSCamera::setTarget(glm::vec3 target)
      {
        ...
        if (!glm::epsilonEqual(targetLength, 0.0f, math::kFloatEpsilon))
        {
          ...
          if (!glm::epsilonEqual(projXZLength, 0.0f, math::kFloatEpsilon))
          {
            projXZ *= (1.0f / projXZLength);
          }
          if (!glm::epsilonEqual(projYZLength, 0.0f, math::kFloatEpsilon))
          {
      ...
      glm::vec2 FPSCamera::SmoothMouseFilter::mouseDelta(float dt)
      {
        ...
        float alpha = static_cast<float>(math::expDecayPow2(kExpDecayCoefficient * dt));
      ...
      glm::vec2 FPSCamera::InertiaMouseFilter::mouseDelta(float dt)
      {
        ...
        if (glm::any(glm::greaterThan(glm::abs(mousePosDelta), glm::vec2(math::kFloatEpsilon))))
        {
