...
namespace mk
{
  namespace renderer
  {
    namespace assets
    {
      ...
      gl::CubeMap ResourceLoader::loadCubeMap(const std::string& imageFileName, const std::string& extension)
      {
        ...
        for (std::size_t i = 0; i < cubeMapImagesFilesNames.size(); ++i)
        {
          ...
          cubeMap.setImage(static_cast<GLenum>(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i), width, height, GL_RGB, imageData);
          SOIL_free_image_data(imageData);
        }
